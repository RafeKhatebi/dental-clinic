...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <h1 class="text-3xl font-bold text-gray-800"><?php echo $lang['trend_charts']; ?></h1>
        <form method="GET" class="flex gap-2">
            <select name="months" class="px-4 py-2 border border-gray-300 rounded-lg">
                <option value="3" <?php echo $months == 3 ? 'selected' : ''; ?>>3 <?php echo $current_lang === 'fa' ? 'ماه' : 'Months'; ?></option>
                <option value="6" <?php echo $months == 6 ? 'selected' : ''; ?>>6 <?php echo $current_lang === 'fa' ? 'ماه' : 'Months'; ?></option>
                <option value="12" <?php echo $months == 12 ? 'selected' : ''; ?>>12 <?php echo $current_lang === 'fa' ? 'ماه' : 'Months'; ?></option>
            </select>
            <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-lg"><?php echo $lang['view']; ?></button>
        </form>
    </div>

    <!-- Monthly Revenue Trend -->
    <div class="bg-white rounded-lg shadow-sm p-6">
        <h2 class="text-lg font-semibold text-gray-800 mb-4"><?php echo $lang['monthly_revenue_trend']; ?></h2>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <!-- Patient Growth -->
        <div class="bg-white rounded-lg shadow-sm p-6">
            <h2 class="text-lg font-semibold text-gray-800 mb-4"><?php echo $lang['patient_growth']; ?></h2>
            <canvas id="patientGrowthChart"></canvas>
        </div>

        <!-- Service Categories -->
        <div class="bg-white rounded-lg shadow-sm p-6">
            <h2 class="text-lg font-semibold text-gray-800 mb-4"><?php echo $lang['service_categories']; ?> (<?php echo $lang['current_month']; ?>)</h2>
...
new Chart(document.getElementById('patientGrowthChart'), {
    ...
    data: {
        ...
        datasets: [{
            label: '<?php echo $lang['total_patients']; ?>',
...
addBreadcrumb('<?php echo $lang['trend_charts']; ?>');
</script>
